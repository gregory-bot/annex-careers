...
def build_ats_cv_pdf(
    ...
    if parsed.summary:
        ...
        # Every sentence is kept; only the order changes.
        summary_body = " ".join(_rank_by_keywords(sentences, matched_lower))
        if job_title:
    ...
    if parsed.certifications:
        story += _section_header("Certifications")
        items = [ListItem(Paragraph(_highlight_relevant(c, matched_lower), BULLET_STYLE), leftIndent=10) for c in parsed.certifications]
        story.append(ListFlowable(items, bulletType="bullet", start="•", leftIndent=12, spaceBefore=2, spaceAfter=6))

    if parsed.projects:
        story += _section_header("Projects")
        items = [ListItem(Paragraph(_highlight_relevant(p, matched_lower), BULLET_STYLE), leftIndent=10) for p in parsed.projects]
        story.append(ListFlowable(items, bulletType="bullet", start="•", leftIndent=12, spaceBefore=2, spaceAfter=6))

    # Sections we don't structure (Languages, Awards, Referees...) are carried
    # over verbatim under the candidate's own heading.
    for heading, body in parsed.extra_sections:
        story += _section_header(heading)
        for line in body:
            story.append(Paragraph(_escape(line.lstrip("-•* ")), BODY_STYLE))
        story.append(Spacer(1, 4))
    ...
    # Flows onto a second page when the source CV needs it, rather than
    # shrinking a full work history into unreadably small type.
    doc.build(story)
    return buf.getvalue()
